...
class YuzuMainlineGenerator(Generator):
    ...
    def generate(self, system, rom, playersControllers, metadata, guns, wheels, gameResolution):
        ...
        if os.path.exists("/userdata/system/switch/suyu.AppImage"):
            ...
            os.chmod("/userdata/system/switch/suyu.AppImage", st.st_mode | stat.S_IEXEC)

            #chmod suyu and suyuQL app
            st = os.stat("/userdata/system/switch/extra/batocera-config-suyuQL")
            os.chmod("/userdata/system/switch/extra/batocera-config-suyuQL", st.st_mode | stat.S_IEXEC)
            st = os.stat("/userdata/system/switch/extra/batocera-config-suyu")
            os.chmod("/userdata/system/switch/extra/batocera-config-suyu", st.st_mode | stat.S_IEXEC)
        ...
        if os.path.exists("/userdata/system/.local/share/suyu"):
            if not os.path.islink("/userdata/system/.local/share/suyu"):
                shutil.rmtree("/userdata/system/.local/share/suyu")
        
        if not os.path.exists("/userdata/system/.local/share/suyu"):
            st = os.symlink("/userdata/system/configs/yuzu","/userdata/system/.local/share/suyu")
        ...
        if os.path.exists("/userdata/system/.config/suyu"):            
            if not os.path.islink("/userdata/system/.config/suyu"):
                shutil.rmtree("/userdata/system/.config/suyu")

        if not os.path.exists("/userdata/system/.config/suyu"):
            st = os.symlink("/userdata/system/configs/yuzu","/userdata/system/.config/suyu")
        ...
        if not os.path.exists("/userdata/system/.cache/suyu"):
            os.mkdir("/userdata/system/.cache/suyu")

        #remove game_list if it exists and isn't a link
        if os.path.exists("/userdata/system/.cache/suyu/game_list"):            
            if not os.path.islink("/userdata/system/.cache/suyu/game_list"):
                shutil.rmtree("/userdata/system/.cache/suyu/game_list")

        if not os.path.exists("/userdata/system/.cache/suyu/game_list"):
            st = os.symlink("/userdata/saves/yuzu","/userdata/system/.cache/suyu/game_list")
        ...
        if system.config['emulator'] == 'suyu-dev':
            commandArray = ["/userdata/system/switch/suyu.AppImage", "-f",  "-g", rom ]
